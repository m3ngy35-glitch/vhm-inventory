...
// Short descriptions just print — nothing to compact. A long one only
// takes up a couple of lines by default (as a <summary>, same collapsed-
// by-default pattern as the variant quick-edit table below it), and
// clicking it expands to the full text right there on the card.
const DESC_COMPACT_LEN = 100;
function renderDescription(description) {
  if (!description) return "";
  if (description.length <= DESC_COMPACT_LEN) {
    return `<p class="card-description muted">${escapeHtml(description)}</p>`;
  }
