...
function renderProductCard(p) {
  ...
  return `
    <div class="product-card" data-id="${p.id}">
      ...
      ${storageLocations.length || sampleLocations.length ? `
        <div class="card-locations">
          ${storageLocations.map(loc => `<span class="loc-chip loc-chip-storage">${escapeHtml(loc)}</span>`).join("")}
          ${sampleLocations.map(loc => `<span class="loc-chip loc-chip-sample">Sample: ${escapeHtml(loc)}</span>`).join("")}
        </div>
      ` : ""}
      <div class="card-body">
            ...
            <tbody>
              ${(p.variants || []).map((v, i) => `
                <tr>
                  ...
                  <td><input class="inline-price" type="number" min="0" step="0.01" data-pid="${p.id}" data-idx="${i}" value="${v.price ?? 0}" /></td>
                  <td><input class="inline-location loc-pill loc-storage" list="storage-locations-quickedit" data-pid="${p.id}" data-idx="${i}" value="${escapeHtml(v.location)}" placeholder="A1" title="Primary storage (actual stock)" /></td>
                  <td><input class="inline-location loc-pill loc-sample" data-pid="${p.id}" data-idx="${i}" value="${escapeHtml(v.sample_location)}" placeholder="e.g. Front window" title="Sample/display piece only" /></td>
                </tr>
              `).join("")}
            </tbody>
          </table>
