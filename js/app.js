      ...
      <div class="card-body">
        ...
        <details class="variant-inline">
          ...
          <table class="variant-inline-table">
            <thead><tr><th>Size</th><th>Color</th><th>POS</th><th>Stock</th><th>Price</th><th>Location</th></tr></thead>
            <tbody>
              ${(p.variants || []).map((v, i) => `
                <tr>
                  <td>${v.size}</td>
                  ...
                  <td class="muted">${escapeHtml(v.location || "—")}</td>
                </tr>
              `).join("")}
            </tbody>
          </table>
...
function wireCardEvents(results) {
  ...
  // Inline quick-edit: POS code, stock, and price, debounced-ish on blur/enter.
  // Location isn't editable here on purpose — it's set by the Live tab's
  // Scan In flow, which is the one place staff actually stand next to the
  // storage bin they're recording; editing it from the desk here would
  // invite it drifting out of sync with where the item really is.
