...
import { SIZES, STORAGE_LOCATIONS, SAMPLE_LOCATIONS } from "./constants.js";
...
async function loadMeta() {
  ...
  const sampleLocList = document.getElementById("sample-locations-quickedit");
  if (sampleLocList && !sampleLocList.childElementCount) {
    sampleLocList.innerHTML = SAMPLE_LOCATIONS.map(loc => `<option value="${loc}"></option>`).join("");
  }
}
            ...
            <tbody>
              ${(p.variants || []).map((v, i) => `
                <tr>
                  ...
                  <td data-label="Sample"><input class="inline-location loc-pill loc-sample" list="sample-locations-quickedit" data-pid="${p.id}" data-idx="${i}" value="${escapeHtml(v.sample_location)}" placeholder="e.g. Front window" title="Sample/display piece only" /></td>
                </tr>
              `).join("")}
            </tbody>
          </table>
