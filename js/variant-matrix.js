...
import { SIZES, VARIANT_TYPES, emptyVariant, STORAGE_LOCATIONS, SAMPLE_LOCATIONS } from "./constants.js";
...
export function renderVariantTable(variants) {
  ...
  const rows = variants.map((v, i) => `
    <tr data-idx="${i}">
      ...
      <td data-label="POS Code"><input class="v-pos" data-idx="${i}" type="text" value="${escapeHtml(v.pos_code)}" placeholder="P001" /></td>
      <td data-label="Alt Codes"><input class="v-alt" data-idx="${i}" type="text" value="${escapeHtml((v.alternate_codes || []).join(", "))}" placeholder="ALT-001, POS-001-A" /></td>
      <td data-label="Stock"><input class="v-stock" data-idx="${i}" type="number" min="0" value="${v.stock_quantity}" /></td>
      <td data-label="Price"><input class="v-price" data-idx="${i}" type="number" min="0" step="0.01" value="${v.price}" /></td>
      <td data-label="Storage"><input class="v-location" data-idx="${i}" type="text" list="vm-storage-locations" value="${escapeHtml(v.location)}" placeholder="A1" /></td>
      <td data-label="Sample"><input class="v-sample-location" data-idx="${i}" type="text" list="vm-sample-locations" value="${escapeHtml(v.sample_location)}" placeholder="e.g. Front window" /></td>
      <td class="v-remove-cell"><button class="btn-icon v-remove" data-idx="${i}" title="Remove variant">&times;</button></td>
    </tr>
  `).join("");

  return `
    <datalist id="vm-storage-locations">
      ${STORAGE_LOCATIONS.map(loc => `<option value="${loc}"></option>`).join("")}
    </datalist>
    <datalist id="vm-sample-locations">
    ...
    <table class="variant-table">
      <thead>
        <tr>
          ...
          <th>Alternate Codes</th><th>Stock</th><th>Price</th>
          <th>Storage</th><th>Sample</th><th></th>
        </tr>
      </thead>
      <tbody>${rows}</tbody>
    </table>
...
export function readVariantTable(container, baseVariants) {
  ...
  container.querySelectorAll(".v-location").forEach(el => {
    variants[+el.dataset.idx].location = el.value.trim();
  });
  container.querySelectorAll(".v-sample-location").forEach(el => {
    variants[+el.dataset.idx].sample_location = el.value.trim();
  });
  return variants;
}
