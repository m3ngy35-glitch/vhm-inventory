...
export function openProductForm({ product, categories, tags, onSaved }) {
  ...
  overlay.innerHTML = `
    <div class="modal modal-lg">
      ...
      <div class="modal-body">
        ...
        <label>Colors (name and hex, one per line: <code>Navy Blue, #000080</code>)
          <textarea id="f-colors" rows="2" placeholder="Navy Blue, #000080&#10;Black, #111111"></textarea>
        </label>
  ...
  overlay.querySelector("#f-generate").onclick = () => {
    ...
    const colors = overlay.querySelector("#f-colors").value.split("\n")
      .map(line => line.trim()).filter(Boolean)
      .map(line => {
        const [name, hex] = line.split(",").map(s => s.trim());
        return { name: name || "Unnamed", hex: hex || "#000000" };
      });
    if (!sizes.length || !colors.length) {
      toast("Pick at least one size and one color first", "error");
      return;
    }
